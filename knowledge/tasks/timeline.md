...
- 下一步：重启客户端并悬停未知价格物品；验证无价格 tooltip 不再出现；继续检查有价格物品仍显示 `参考价格 : xx落叶币`。
...
## 2026-06-23 21:30 +08:00 - 统一参考价格文案

- 当前阶段：运行期展示文案微调。
- 本段重点：tooltip 文案改为 `参考价格 : xx落叶币`；默认单位从 `枫币` 改为 `落叶币`；命令反馈同步改为“参考价格”。
- 关键决策：只改展示层和默认配置，不改内部价格计算与协议结构；已有客户端本地 `arcvaluecalc-common.toml` 若手动改过旧单位，需要按实际配置覆盖。
- 验证记录：`./gradlew.bat build` 通过；`arcvaluecalc-0.2.0.jar` 已覆盖到客户端 mods 目录。
- 下一步：重启客户端并执行 `/arcvalue reload`；确认 tooltip 显示为 `参考价格 : xx落叶币`。
